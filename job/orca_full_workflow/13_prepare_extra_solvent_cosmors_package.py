#!/usr/bin/env python3
"""Prepare ORCA COSMORS single-point jobs for the two extra solvents."""
...
PACKAGE_DIR = ROOT / "pet_extra_solvent_cosmors_package"

JOBS = [
    {
        "source_dir": "32-C5H9NO",
        "trajectory": "32-C5H9NO_opt_trj.xyz",
        "final_xyz": "32-C5H9NO_opt_final.xyz",
        "basename": "nmp",
        "job_name": "cosmo_nmp",
        "ncores": 4,
    },
    {
        "source_dir": "33-C3H8O",
        "trajectory": "33-C3H8O_opt_trj.xyz",
        "final_xyz": "33-C3H8O_opt_final.xyz",
        "basename": "isopropanol",
        "job_name": "cosmo_ipa",
        "ncores": 4,
    },
]
...
README_TEXT = """# Extra solvent ORCA COSMORS package
...
def parse_xyz_frames(path):
    ...
    while index < len(lines):
        ...
        try:
            ...
            raise ValueError(
                "Expected atom count at line {} in {}".format(index + 1, path)
            ) from exc
        ...
        if last_coord_index > len(lines):
            raise ValueError(
                "Truncated XYZ frame starting at line {} in {}".format(index + 1, path)
            )
        ...
        if len(coords) != natoms:
            raise ValueError(
                "Frame at line {} has wrong atom count in {}".format(index + 1, path)
            )
...
def main():
    ...
    for job in JOBS:
        ...
        if not trajectory.is_file():
            raise FileNotFoundError(
                "{} not found. Run the ORCA optimization first and download the "
                "*_opt_trj.xyz file.".format(trajectory)
            )
    ...
    print("Generated extra solvent COSMORS package:")
