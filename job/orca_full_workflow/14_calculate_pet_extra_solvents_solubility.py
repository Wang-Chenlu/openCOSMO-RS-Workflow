#!/usr/bin/env python3
"""Calculate PET solubility in NMP and isopropanol from ORCA COSMO files."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate PET solubility in NMP and isopropanol."
    )
    parser.add_argument(
        "--delta-h-fusion-j-g",
        type=float,
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "pet_extra_solvent_solubility_results"),
...
def collect_solvents():
    base = ROOT / "pet_extra_solvent_cosmors_package"
    fallback = ROOT
    return [
        (
            "NMP",
            require_first_existing(
                [
                    base / "nmp.solute.orcacosmo",
                    fallback / "nmp.solute.orcacosmo",
                ]
            ),
        ),
        (
            ...
            require_first_existing(
                [
                    base / "isopropanol.solute.orcacosmo",
                    fallback / "isopropanol.solute.orcacosmo",
                ]
            ),
        ),
    ]
...
def require_first_existing(paths):
    for path in paths:
        path = Path(path).resolve()
        if path.is_file():
            return path
    raise FileNotFoundError(
        "None of these files exist: {}".format(", ".join(str(path) for path in paths))
    )
...
def solve_solubility(solute_file, solvent_file, delta_h_fusion_j_mol, t_fusion_k, temperature_k):
    ...
    x_inf_raw = math.exp(rhs - ln_gamma_inf)
    x_inf = min(max(x_inf_raw, EPS), 1.0)
...
def main():
    ...
    per_frame_csv = output_dir / "pet_extra_solvent_solubility_per_frame.csv"
    ...
    for solvent_name, _ in solvents:
        ...
        for quantity in ["ln_gamma_inf", "x_inf_raw", "x_inf", "g_per_g_inf", "x_iter", "g_per_g_iter"]:
    ...
    summary_csv = output_dir / "pet_extra_solvent_solubility_summary.csv"
    write_csv(
        summary_csv,
        summary_rows,
        [
    ...
    print("{:<14}{:>16}{:>16}{:>16}{:>16}".format(
        "solvent", "mean g/g_iter", "median g/g_iter", "min", "max"
    ))
    print("-" * 78)
    for solvent_name, _ in solvents:
        rows = [row for row in per_frame_rows if row["solvent"] == solvent_name]
        stats = summarize(rows, "g_per_g_iter")
        print("{:<14}{:>16.6g}{:>16.6g}{:>16.6g}{:>16.6g}".format(
